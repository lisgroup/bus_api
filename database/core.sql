...
CREATE TABLE `notice` (
    ...
    `arrival_early_minute` tinyint NOT NULL DEFAULT 0 comment '到站最早分钟数',
    `arrival_last_minute` tinyint NOT NULL DEFAULT 0 comment '到站最晚分钟数',
    `notice_time` tinyint NOT NULL DEFAULT 0 comment '通知次数',
    `created_at` datetime DEFAULT NULL,
    `updated_at` datetime DEFAULT NULL,
    `deleted_at` datetime DEFAULT NULL,
    PRIMARY KEY (`id`),
